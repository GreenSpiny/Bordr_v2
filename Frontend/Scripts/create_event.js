...
function append(event) {
	console.log("post event",event);
  	$.post('http://localhost:3000/appendList', {collection: "users", field: "hosting_events", entity: current_user.user._id, field_value: event}).done( function(response) {
    	console.log(response);
    });
    $.post('http://localhost:3000/appendList', {collection: "users", field: "participating_events", entity: current_user.user._id, field_value: event}).done( function(response) {
    	console.log(response);
    });
}